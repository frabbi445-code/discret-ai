...
# ২. এপিআই কি কনফিগারেশন এবং পারফেক্ট মডেল পাথ ভেরিফিকেশন লজিক
try:
...
if GEMINI_API_KEY:
    ...
    try:
        ...
        # 🛠️ ফিক্সড মডেল পাথ: models/ প্রিফিক্স যোগ করা হলো যেন v1beta কনফ্লিক্ট ও 404 এরর না আসে
        ai_model = genai.GenerativeModel('models/gemini-1.5-flash')
        
        # ১-টোকেন রিয়েল লাইভ টেস্ট পিং (নিশ্চিত হওয়ার জন্য যে কী-টি সত্যিই কাজ করছে)
...
# 🟢/🔴 ডাইনামিক ডট ইন্ডিকেটর লজিক (যা কুয়েরি ও রেসপন্সের ওপর ভিত্তি করে লাইভ চেঞ্জ হবে)
if ai_ready:
    ...
    st.markdown('<div class="status-panel" style="background-color: rgba(244, 63, 94, 0.1); border: 1px solid #f43f5e; color: #f43f5e !important;">🔴 Core AI Engine: OFFLINE (Smart Hybrid Fallback Mode Enabled)</div>', unsafe_allow_html=True)
...
if st.button("Generate Detailed AI Lecture Note", use_container_width=True):
    with st.spinner(f"✨ Compiling notes for {lesson_topic}..."):
        content = None
        # যদি এপিআই কী রেডি থাকে, তবে সরাসরি জেমিনি থেকে লাইভ জেনারেট হবে
        if ai_ready and ai_model:
            try:
                prompt = f"Write a highly comprehensive university undergraduate lecture note on '{lesson_topic}' in a professional mix of Bengali and English. Include definitions, 2 solved mathematical examples using LaTeX formatting, and textbook references with URLs."
                resp = ai_model.generate_content(prompt)
                content = resp.text
            except Exception:
                content = None
        
        # কোনো কারণে এপিআই ফেল করলে বা ৪MD এরর দিলে লোকাল সুপার-ডিটেইলড ব্যাংক ব্যাকআপ লোড হবে
        if not content:
            content = global_lessons.get(lesson_topic, "### Lecture database sync complete.")
...
# 🤖 ৮. রিয়েল পিডিএফ/টেক্সট রিডার অ্যান্ড এআই এক্সপেইনার (ক্রাশ প্রুফ মেথড)
...
# 🧠 ১০. ডাইনামিক ফিল্টার সংবলিত ১০-কোয়েশ্কেন মক টেস্ট ল্যাব
